...
This document is based on Angrist and Pischke, *Mostly harmless econometrics* (2008), and the lecture slides of Kosuke Imai and Chad Hazlett.
...
<p align="center">
 ...
 - Assumptions:
     1. Exclusion restriction: getting encouragement email itself has no effect on participation in the events other than through reading the message
     2. Monotonicity: getting email would never discourage anyone from participating in the events
...
## Data
mcclendon <- read_dta("McClendon_2014/data/lgbtrally_anondata.dta")
...
```{r}
#data with all covariates is available with this link (QOB.rar): 
#https://economics.mit.edu/faculty/angrist/data1/data/angkru1991
#variable descriptions are found with this link: https://economics.mit.edu/files/5354
qob_data <- read.table("Angrist_and_Krueger_1991/data/QOB.txt", 
                       header=FALSE, stringsAsFactors = FALSE)
qob_data <- qob_data[, c(1,2,4,5,6,9,10,11,12,13,16,18,19,20,21,24,25,27)]
names(qob_data) <- c("age", "ageq", "educ", "enocent", "esocent", "lwklywge", "married", "midatl",
                     "mt", "neweng", "census", "qob", "race", "smsa", "soatl", "wnocent", "wsocent", "yob")
...
qob_data <- qob_data %>%
  mutate(cohort = case_when(yob %in% 30:39~ 3039,
         ...
         ageq_sq = ageq^2,
         #year-quarter variable which factor() can turn into dummy variables (for regression)
         yqob = paste(yob, qob, sep='Q')) %>% 
  filter(cohort == 3039) #use only cohort of those born in 1930s

qob_means <- qob_data %>% group_by(yob, qob) %>% summarise_all(funs(mean))
qob_means$yqob <- lubridate::ymd(paste0("19", qob_means$yob, qob_means$qob*3), truncated = 2) #add dates
...
```
 ...
 - Settings
     + Linear model with covariates (in matrix form): $$\mathbf{Y}=\X\beta + \epsilon,$$ where $\E(\epsilon)=\mathbf{0}_n$.
     
     + "Endogeneity" means $\E(\epsilon|\X) \neq 0$
     
     + $\Z$ is instrument that satisfies 1) exogeneity $E(\epsilon_i \mid \Z)=0$, 2) exclusion restriction ($Z_i$ affects outcome $Y_i$ only through $X_i$), and 3) rank condition ($\Z'\X$ and $\Z'\Z$) have full rank.
     
     + In experimental setting, $\X$ is the treatment and pretreatment covariates and $\Z$ is the randomized encouragement.
     
 - 2-Stage Least Squares estimator (IV estimator): 
 \begin{align*}
 \hat{\beta}_{IV} &= (\hat{\X}'\hat{\X})^{-1}\hat{\X}'\mathbf{Y} \\
 &= (\X' \Z(\Z'\Z)^{-1} \Z'\X)^{-1} \X'\Z(\Z'\Z)^{-1}\Z'\mathbf{Y},
 \end{align*}
 where $\mathbf{P_\Z} = \Z(\Z'\Z)^{-1}\Z'$ is the projection matrix (onto $span(\Z)$) and $\hat{\X} = \mathbf{P_\Z}\X$. Therefore, by predicting $\X$ using $\Z$, we "purge" endogeneity in $\X$ (remove non-random part in $\X$). Also, when the number of instrumental variables and the number of endogenous components (generally treatments we're interested in), it can be further simplfied to:
 \begin{align*}
 \hat{\beta}_{IV} &= (\X' \Z(\Z'\Z)^{-1} \Z'\X)^{-1} \X'\Z(\Z'\Z)^{-1}\Z'\mathbf{Y} \\
 &= (\Z'\X)^{-1}(\Z'\Z)(\X'\Z)^{-1}\X'\Z(\Z'\Z)^{-1}\Z'\mathbf{Y} &(\X'\Z, \Z'\Z, \X'\Z) \text{ are square} \\
 &= (\Z'\X)^{-1}\Z'\mathbf{Y}
 \end{align*}
 
 Intuitively, IV tries to solve the unobserved confounding problem by using only part of the variability in schooling years (a "random" part that is uncorrelated with potential confounders) to estimate the relationship between education and earnings.
 
 - Standard errors
     1. Under homoskedasticity ($\V[\epsilon|Z]=\sigma^2 I_n$): $$\widehat{\V[\hat{\beta}_{\text{IV}}]} = \hat{\sigma}^2 \{\X'\Z(\Z'\Z)^{-1}\Z'\X\}^{-1} = \hat{\sigma}^2 \{\hat{\X}'\hat{\X}\}^{-1},$$ where $\hat{\sigma}^2 = \frac{\mathbf{e}'\mathbf{e}}{n-k}$ and the residuals are $\mathbf{e} = \mathbf{Y} - \X\hat{\beta}_{\text{IV}}$ (not $\hat{\X}$)
     
     2. "Sandwich" HC estimator:
 $$
 \begin{align*}
 \underbrace{\widehat{\V(\hat{\beta}_{\text{IV}})}}_{\text{sandwich}} &= \underbrace{(\hat{\X}'\hat{\X})^{-1}}_{\text{bread}} \underbrace{\hat{\X}' \color{red}{\widehat{\V[\epsilon|\X]}} \hat{\X} }_{\text{meat}} \underbrace{(\hat{\X}'\hat{\X})^{-1}}_{\text{bread}} \\
 &= (\hat{\X}'\hat{\X})^{-1}  \hat{\X}'\color{red}{\text{diag}(\hat{\epsilon}_i^2}) \hat{\X} (\hat{\X}'\hat{\X})^{-1}
 \end{align*}
 $$
 
 - The estimator seems complicated, but the implementation is easy:
     1. Regress $\X$ on $\Z$ and get the predicted values $\hat{\X}$
     2. Regress $\mathbf{Y}$ on $\hat{\X}$
...
```
 ...
 <br>
  ...
  - More instruments?
     + Specifically, instruments are quarter-of-birth dummies interacted with year-of-birth dummies (`qob` $\times$ `yob` $\rightarrow$ `yqob`), rather than just quarter-of-birth. Maybe he idea is that the quarter of birth effect (`qob`) may differ by cohort (`yob`) (For columns 5-8, they additionally load up the instruments with the quarter of birth $\times$ state)
     + Pros: more instruments have more variation, so (the predicted values of) education also have more variation, which could improve precision (smaller standard errors.)
     + Cons: many of fine-grained instruments are now weakly correlated with education years (weaker correlation in some cohort)--we added a bunch of noise in the first stage. It could lead to so-called "weak instrument." 
     + Bound et al. ([1995](https://www.tandfonline.com/doi/abs/10.1080/01621459.1995.10476536) tried to understand what IV was identifying when the first stage was "weak," as it was when this Angrist and Krueger (1991) loaded up their instruments by interacting `qob` with `yob`, many of which were very weak. The takeaway of the literature is that a mild weak instruemnt can lead to a severe bias in 2SLS estimate. A recent review on this topic can be found ([here](https://www.annualreviews.org/doi/abs/10.1146/annurev-economics-080218-025643)
