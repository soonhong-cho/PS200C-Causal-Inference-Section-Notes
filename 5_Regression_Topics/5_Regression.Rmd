---
title: '200C Section 4: Regression'
author: 'Soonhong Cho'
date: "April 29, 2022"
output:
...
This document is based on Imbens and Rubin, *Causal inference in statistics, social, and biomedical sciences* (2015), Aronow and Miller, *Foundations of agnostic statistics* (2019) Kosuke Imai's lecture note, and Erin Hartman's lecture note.



# Error structure, "robust" standard errors in OLS
 ...
 - Let's start with the variance-covariance matrix of the OLS estimator $\hat{\beta}$ (in Pset 1).
     + First, note that
     $$
     \begin{align*} 
     \hat{\beta} &= (\X'\X)^{-1}\X'Y \\
...
```
 ...
 $$
     ...
     + **HC1** (Stata's default: `regress y x1 x2, robust`): The meat for HC1 is $$\frac{n}{n-k} \X' \text{diag}(e_i^2) \X,$$ where $n$ is size and $k$ is the number of parameters. The more data available relative to the number of parameters estimated, the closer it is to 1.
...
As the left plot shows, more than 5\% of the CIs (56\%!!!) does not include the true values, meaning that we reject the null of no effect more than 5\% of the time (more chance of getting "significant" results). That is, if we analyze clustered-data with OLS, we may incorrectly reject the null hypothesis more than we should since the standard error is not clustered. When we cluster the standard errors, the CIs achieve the target coverage rate 95\% as the right plot suggests. This contrast warns us that STANDARD ERRORS MATTER.
  ...
  - Ok, let's wrap up. We've talked about what assumptions go into what SEs and which SEs are appropriate in what circumstance.
    ...
    4. **Bootstrap** can be a useful alternative in any case, with same concerns (e.g., if data has clustering, do block (cluster) bootstrap).



```{r, eval=FALSE}
## clustering at district level from last section
...
# Simple Linear Regression and Causality

## Agnostic view
 - It's a good opportunity to reflect on the most commonly used statistical tool in the social sciences, regression. Many statisticians including Freedman have warned that regression is not justified by randomization, but others (Angrist and Pischke's mostly harmless) argued that once a causal effect can be identified, then a regression is useful to approximately estimate that effect. Let's think about regression from the perspective of causal inference, with potential outcome model.
 
 - First of all, without believing the usual modeling assumptions (the distribution of data $Y$ is normal with a mean that is a linear function of the covariates $X$, with a common variance, $\sigma^2$, across units, etc.), we can consider regression as just a <span style="color:red;">conditional expectation function (CEF)</span>: $$\E[Y|X] = f(X) = X \beta,$$ so we're try to estimate how the distribution of $Y_i$ changes with a covariate $X_i$. Mechanically, running a regression is just estimating a conditional expectation function, the <span style="color:red;">"best" linear approximation</span> (in terms of minimum MSE) to the true relationship between variables (whether it's linear or non-linear). We don't have to take all the modeling assumptions of the linear model seriously to use the regression machinery as an estimation technique to estimate causal effects of interest. This perspective is called *agnostic* view on regression (Aronow and Miller book, the textbook for PS200B).
 
 - From causal inference perspective, then the question is: when does knowing the CEF give us something about causality? When can we interpret the "beta hat" as causal effects?


## Causal Interpretation of Simple Linear Regression

 - To connect regression to causal interpretation, let's link potential outcomes to observed outcomes. It is done by the consistency assumption, so let's rewrite it to be a regression model:
 \begin{align*}
 Y_i &= D_iY_{1i} + (1-D_i)Y_{0i} \\
 &= Y_{0i} + (\underbrace{Y_{1i} - Y_{0i}}_{\tau_i}) D_i     &\text{(rearrange)} \\
 &= \E[Y_{0i}] + \tau_i D_i + (Y_{0i} - \E[Y_{0i}])     &\text{(add and subtract } \E[Y_{0i}]) \\
 &= \E[Y_{0i}] + \tau_{ATE} D_i + (Y_{0i} - \E[Y_{0i}]) + (\tau_i - \tau_{ATE})D_i     &\text{(add and subtract } \tau_{ATE}D_i) \\
 &=\E[Y_{0i}] + \tau_{ATE}D_i + \color{red}{\eta_i} \\
  \\
 {\Longleftrightarrow} Y_i &= \alpha + \beta X_i + \color{blue}{\epsilon_i}    &\text{[reparameterize to be a regression]}
 \end{align*}
 
 1. First, let's interpret each term. 1) $\E[Y_{0i}]$ is the average untreated potential outcomes. It's unobservable quantity: average  outcome that  would have been observed had every unit received treatment. 2) $\tau_{ATE}$ is the average treatment effect. $\eta_i$ is an error term. It consists of two components: $$\eta_i = \underbrace{(Y_{0i} - \E[Y_{0i}])}_{(1)} + \underbrace{(\tau_i - \tau_{ATE})D_i}_{(2)},$$ where (1) is  the "baseline" variation in the outcome across units, and (2) variation in treatment effect across units.
 
 2. We can see that $\alpha = \E[Y_{0i}]$ and $\beta$ captures the $\tau_{ATE}$, when the error terms are equal, i.e., $\eta_i = \epsilon_i$.
 
 3. In regression framework, the most important assumption in terms of causation is called <span style="color:red;">strict exogeneity</span> (or "Conditional Independence Assumption"): $$\E[\epsilon_i | X] = \E[\epsilon_i] =  0,$$ which implies that there is no unmeasured confounder. If this assumption holds true, the least squares  estimate $\hat{\beta}_{OLS}$ is unbiased for $\beta$, and we could give it a causal claim. \
 As you can  see, it corresponds to the ignorability assumption ($\{ Y_{1i}, Y_{0i} \} \independent D_i$), which can be guaranteed by randomization of treatment as $$\E[Y_{di}|D] = \E[Y_{di}]  \Longleftrightarrow  \E[\eta_{di}|D_i] = \E[\eta_{di}] = 0.$$ Therefore, **under strict exogeneity, like under ignorability, we can run a regression to estimate the AT** (note: strictly speaking, we needed the linearity of the treatment on outcome, a typical modeling assumption, but binary $D_i$ automatically satisfies it. If effect of the treatment is truly linear, our result holds with ordinal or continuous treatment variable.)
 
 4. Error decomposition: but the interpretation could be different, depending on the assumption on error structure. Note that the **random sampling** of units ($\{Y_{1i}, Y_{0i}\} \independent  \{Y_{1j}, Y_{0j}\}$, hence $\{\epsilon_{1i}, \epsilon_{0i}\} \independent  \{\epsilon_{1j}, \epsilon_{0j}\}$) implies the **no correlation (of error) between observations** in regression: $\E[\epsilon_i \epsilon_j|X]=0$ for $i \neq j$, hence $Cov(\epsilon_i, \epsilon_j)=0$ for $i \neq j$. Then, we don't have to care about off-diagonal terms in variance-covariance matrix.
 
      + 1) **<span style="color:red;">Homoskedasticity</span>**: $\E[\epsilon_i^2|X]=\sigma^2$ (which implies $\V(\epsilon_i|X)=\sigma^2$ with exogeniety $\E[\epsilon_i|X]=0$). We effectively assume that the conditional variance of errors in the treated group is the same as the conditional variance of errors in the control group:
      \begin{align*}
      \V(\epsilon_i|X)=\sigma^2 &\Longrightarrow \V(\eta_i|D_i=1) = \V(\eta_i|D_i=0)
      &\Longrightarrow (\tau_i-\tau_{ATE})D_i \text{ must be zero} \qquad \text{ (2nd component in } \eta_i, \text{ i.e., }(2)) \\
      &\Longleftrightarrow \tau_i - \tau_{ATE} = 0 \\
      &\Longleftrightarrow \tau_i = \tau_{ATE}
      \end{align*}
      Therefore, homoskedasticity assumption in regression implies **<span style="color:red;">constant additive unit treatment effect</span>** ($\tau_i=\tau_{ATE}$): $\hat{\beta}$ estimates a constant additive effect, the same effect for all units (like the sharp null).

      <br>
      + 2) **<span style="color:red;">Heteroskedasticity</span>**: we can relax homoskedasticity by allowing $\V(\epsilon_i|X)=\sigma_i^2$ to vary at each strata of $X$. In binary treatment case, it means unequal variance across treatment conditions:
      \begin{align*}
      \underbrace{\V(\epsilon_i|D_i=1)}_{\sigma_1^2} \neq \underbrace{\V(\epsilon_i|D_i=0)}_{\sigma_0^2} \quad &\Longrightarrow  \V(\eta_i|D_i=1) \neq \V(\eta_i|D_i=0) \\
      &\Longleftrightarrow (\tau_i-\tau_{ATE})D_i \text{ need not be zero} \qquad \text{ (2nd component in } \eta_i, \text{ i.e., }(2)) \\
      &\Longleftrightarrow \tau_i \neq \tau_{ATE}
      \end{align*}
      Thus, heteroskedasticity assumption in regression encodes **<span style="color:red;">heterogenous treatment effect</span>** ($\tau_i \neq \tau_{ATE}$): $\hat{\beta}$ is an unbiased estimator of the (sample) ATE. \
      Note: when moving from homoskedasticity to heteroskedasticity, we have the same point estimate $\hat{\beta}$ (the interpretation of $\hat{\beta}$ changes though), since $$\E[\tau_i] = \E[y_{1i} -  y_{0i}] = \alpha + \beta + \E[\epsilon_i]  - (\alpha + \E[\epsilon_i]) = \beta$$ holds regardless of the assumption on error structure. We are changing only the standard errors.
   
 -  Summary: homoskedasticity implies the equal variance of potential outcomes across treatment groups and the constant additive treatment effect, which is usually implausible. Heteroskedasticity-robust variance estimator can relax this assumption by allowing for unequal variance of potential outcomes across treatment/control groups.
...
Q) When you have experimental data, it is better to run a simple regression of the outcome variable on the binary treatment variable than to use the Difference-in-Means estimator. This is because the ordinary least squares (OLS) regression yields the most efficient estimator and thus the smallest standard error for the average treatment effect.

A) False. The OLS slope coefficient is *numerically* identical to the difference-in-means estimator when the treatment is binary and no other predictors are included in the model. Therefore, their sampling variances are also identical.
